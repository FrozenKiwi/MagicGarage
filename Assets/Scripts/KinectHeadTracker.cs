...
public class KinectHeadTracker : TrackerBase, SpeechRecognitionInterface
{
    ...
    void Awake()
    {
        ...
        // The only way to know fuh shore Kinect is operating is to \
        // count them (all the other options get confused by sensorAlwaysAvailable);
        IsConnected = Kinect.GetSensorData()?.sensorInterface?.GetSensorsCount() > 0;

        if (IsConnected)
        {
            Debug.Log("Kinect Head Tracker initialized");
        }
